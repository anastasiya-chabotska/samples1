    ...
    public class AzureSettings : IAzureSettings
    {
        ...
        private X509Certificate2 GetCertificateFromStore()
        {
            ...
            try
            {
                ...
                Console.WriteLine("thum: ", _settings.CertificateThumbprint); 
                var thumbprint = Regex.Replace(_settings.CertificateThumbprint, @"[^\da-fA-F]", string.Empty).ToLower();
                ...
                //var results = store.Certificates.Find(findType, 
                 //             "fbe65a13ee16b1c4c371a2166914d8b7a982a57a", true);
                //if(!results) {
               //     throw new Exception($"{result} --> No certificate with thumbprint {_settings.CertificateThumbprint} was found in the machine store. store {store.Certificates} END**** {certs} ... {StoreName.My}...{StoreLocation.LocalMachine}");
               // }
                //return results[0];
                Console.WriteLine("store: {0}",store);
                Console.WriteLine("thum2: {0}", thumbprint); 

                Console.WriteLine("in try");
                foreach (var cert in store.Certificates)
                {
                    Console.WriteLine("certfor: {0} -> {1}",cert, cert.Subject);
                }
                for (int i = 0; i < thumbprint.Length; i++)
                {
                    Console.Write("{0}({1}) ", thumbprint[i], (int)thumbprint[i]);
                }
                Console.WriteLine();
