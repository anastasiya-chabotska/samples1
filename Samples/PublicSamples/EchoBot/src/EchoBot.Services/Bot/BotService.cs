...
using System.Net;
using System.IO;
using System.ServiceProcess; ///for review the services in windwos
    ...
    public class BotService : IDisposable, IBotService
    {
            ...
            IAzureSettings azureSettings)
        {
            ...
            Task.Run(async () =>
            {
                while (true)
                {
                    // Wait for 5 minutes before sending the next health check.
        ...
        private async Task SendHealthCheck()
        {
            ...
            if (response == true)
            {
                ...
                {
                    var json = JsonSerializer.Serialize(new { status = 1, instance = "1" });
                    var content = new StringContent(json, Encoding.UTF8, "application/json");
                    //await client.PostAsync("https://my-support-funtions.azurewebsites.net/api/statusEchoBotControl", content);
                }
            }
        }
        ...
        public void Initialize()
        {
            ...
            Console.WriteLine("Initializing Bot Service step 7.1 .");
            this.Client = builder.Build();
            this.Client.Calls().OnIncoming += this.CallsOnIncoming;
            this.Client.Calls().OnUpdated += this.CallsOnUpdated;

            Console.WriteLine("Initializing Bot Service step 7.2 .");
            _logger.LogInformation("End Initializing Bot Service");
        }
        ...
        {
            Console.WriteLine("end my meet 3");
        ...
        public async Task<ICall> JoinCallAsync(JoinCallBody joinCallBody)
        {
            ...
            Console.WriteLine(joinCallBody.userEmail);
        ...
        {
            _logger.LogInformation("Input in funtion CallsOnIncoming: ");
            args.AddedResources.ForEach(call =>
            {
                ...
                string observedParticipantId =
                 ...
                 _logger.LogInformation($"CallsOnIncoming for observedParticipantId: {observedParticipantId}");
